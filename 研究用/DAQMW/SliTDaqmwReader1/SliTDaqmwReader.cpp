...
static const char* slitdaqmwreader_spec[] =
  {
   "implementation_id", "SliTDaqmwReader",
   "type_name",         "SliTDaqmwReader",
   "description",       "SliTDaqmwReader component",
   "version",           "1.0",
...
SliTDaqmwReader::SliTDaqmwReader(RTC::Manager* manager)
  : DAQMW::DaqComponentBase(manager),
    m_OutPort("slitdaqmwreader_out", m_out_data),
    m_sock(0),
    m_recvByteSize(0),
    ...
    m_firmwareVersion(1)
{
  ...
  // Set OutPort buffers
  registerOutPort("slitdaqmwreader_out", m_OutPort);

  init_command_port();
  init_state_table();
  set_comp_name("SliTDaqmwReader");
}

SliTDaqmwReader::~SliTDaqmwReader()
{
}

RTC::ReturnCode_t SliTDaqmwReader::onInitialize()
{
  if (m_debug) {
    std::cerr << "SliTDaqmwReader::onInitialize()" << std::endl;
  }

  return RTC::RTC_OK;
}

RTC::ReturnCode_t SliTDaqmwReader::onExecute(RTC::UniqueId ec_id)
{
  daq_do();
...
int SliTDaqmwReader::daq_dummy()
{
  return 0;
}

int SliTDaqmwReader::daq_configure()
{
  std::cerr << "*** SliTDaqmwReader::configure" << std::endl;
...
int SliTDaqmwReader::parse_params(::NVList* list)
{
...
int SliTDaqmwReader::daq_unconfigure()
{
  std::cerr << "*** SliTDaqmwReader::unconfigure" << std::endl;
...
int SliTDaqmwReader::daq_start()
{
  std::cerr << "*** SliTDaqmwReader::start" << std::endl;
...
int SliTDaqmwReader::daq_stop()
{
  std::cerr << "*** SliTDaqmwReader::stop" << std::endl;
...
int SliTDaqmwReader::daq_pause()
{
  std::cerr << "*** SliTDaqmwReader::pause" << std::endl;
  
  return 0;
}

int SliTDaqmwReader::daq_resume()
{
  std::cerr << "*** SliTDaqmwReader::resume" << std::endl;
  
  return 0;
}

int SliTDaqmwReader::create_buffer_ipaddress(){
...
void SliTDaqmwReader::read_data_from_detectors()
{
  if( m_debug ) std::cerr << "*** SliTDaqmwReader::read_data_from_detectors" << std::endl;
...
int SliTDaqmwReader::set_data(unsigned int data_byte_size)
{
...
int SliTDaqmwReader::write_OutPort()
{
...
int SliTDaqmwReader::daq_run()
{
  if( m_debug ) std::cerr << "*** SliTDaqmwReader::run" << std::endl;
...
extern "C"
{
  void SliTDaqmwReaderInit(RTC::Manager* manager)
  {
    RTC::Properties profile(slitdaqmwreader_spec);
    manager->registerFactory(profile,
			     RTC::Create<SliTDaqmwReader>,
			     RTC::Delete<SliTDaqmwReader>);
  }
};
