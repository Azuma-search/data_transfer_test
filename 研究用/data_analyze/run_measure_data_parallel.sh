...
for DATANUM in "${DATANUM_LIST[@]}"; do
	...
	OUTPUT_NAME="data/${CURRENT_TIME}_SSD_${DATANUM}_"$RUN_SECONDS"_"
	#OUTPUT_NAME="data/${CURRENT_TIME}_SSD_${DATANUM}_"5"_"
	./run_setup.sh "$BOARD_ID1" "$NUM"
	./run_setup.sh "$BOARD_ID2" "$NUM"
	./run_setup.sh "$BOARD_ID3" "$NUM"
	./run_setup.sh "$BOARD_ID4" "$NUM"
	./slitreader -m 2 -t $((RUN_SECONDS + 1)) -o "$OUTPUT_NAME" -p "$BOARD_ID1" "$BOARD_ID2" "$BOARD_ID3" "$BOARD_ID4"
